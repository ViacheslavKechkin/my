...
Если через NPM - npm i lodash и после нужно подключить - import _ from 'lodash'
...
const askMom = () => {
  ...
  promise.then((result) => console.log(result) ).catch((err) =>  alert(err.messege) );
};
askMom();
...
const app = express();
//дает доступ всем (не безопастно) в виде массива можно в скибки передать хосты
app.use(cors());
...
Today:
- I fixed a lot of problems in my project: changed the color of the snackbar, made the exit button fully functional, made the date get from the backend, changed the background color in inputs on autocomplete, used a collection for the doctor dropdown and much more

Tomorrow:
- I will continue fixing bugs in my project. I will save the token in local starage. I will filter records for a specific user on the backend. And I hope I will have time to make a route to update the record
