...
SHADOW DOM - является технологией браузера
// это способ создать свой, изолированный, DOM для компонента.
// все что мы добавляем является локальным означает, что стили, созданные внутри одного элемента Shadow DOM, 
// изолированы и остаются в рамках этого Shadow DOM
VIRTUAL DOM - концепция реализуется в библиотеках JavaScript на вершине API для браузера
// происходит сравнение DOM-дерева с его виртуальной копией, определяется разница и 
// запускается перерисовка того, что было изменено.
React PORTAL
 - это Компонент который рендорит свое содержимое в произвольную часть DOM дерева
Используется для: модальных и диалоговых окон, всплывающие карточки и всплывающие подсказки.
...
React FRAGMENT 
// Огромный плюс что он не виден в DOM дереве и поэтому не мешает стилизации 
// плюс в отличии от узла div который что то весит, он ничего не требует
REACT KEY  - //реакт сверяет данные при обновлении по ключю
HOC - //принимает передаваемый ему компонент и возвращает новый компонент с расширенной логикой
HOF - //принимает аргументом функцию и производит какие то операции с ней (и возвращает функцию)
React Reconciliation - механизм сверки реактом виртуального DOM с DOM
...
ДЛЯ ОПТИМИЗАЦИИ 
- componentWillUnmount() и useEffect  что бы отменять подписки на что либо при размонтировании
...
//нужны например для проверки ролей
//Делаю enum и в него передаю данные, что бы в дальнейшем не описаться или не допустить какую либо ошибку
enum UserRole {
  Admin, // Admin = 'Admin' можно задать порядок цифр либо что-то в строке
  User,
}
const user = {
  role: UserRole.Admin
}
//проверяю пользователя и данные в enum
...
//Дженерик в функции как аргумент, внутри <> указываем что будем передавать
...
//то метод у переменной id будет общий как для строки и для числа(не будет методов именно к строке или числу) для этого и нужны сущения типов !
(typeof strs === 'object') - условие называется защитником или предохранителем типа (typeGuard)
...
//указываю T диманический тип объекта,  L1 будет получать ключи из T, L2 будет получать ключи из L1
function getMenu<T, L1 extends keyof T, L2 extends keyof T[L1]>(
  obj: T, l1: L1, l2: L2,
) {
  return obj[l1][l2];
}
const res = getMenu(menu, 'analytics', 'data')
